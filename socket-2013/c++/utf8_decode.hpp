...
#ifndef UTF8_DECODE_HPP_
#define UTF8_DECODE_HPP_
...
unsigned int utf8_decode(char *s, unsigned int *pi) {
	...
	if ((s[i] & 128) == 0) {
		...
		c = (((unsigned int) s[i] & 31) << 6) + ((unsigned int) s[i + 1] & 63);
		i += 2;
...
std::string UrlDecode(const std::string& src) {
	...
	for (size_t i = 0; i < srclen; i++) {
		if (src[i] == '%') {
			if (isxdigit((int)src[i + 1]) && isxdigit((int)src[i + 2])) {
				char c1 = src[++i];
				char c2 = src[++i];
